    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        <div className="glass-card p-6 flex flex-col justify-between">
          <span className="text-slate-400 text-xs font-semibold uppercase tracking-wider mb-2">Entradas & Saídas ({mesPassado})</span>
          <div>
        ...
        {/* Saldo do mês */}
        <div className={`glass-card p-6 border ${totais.saldo >= 0 ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-300' : 'bg-rose-500/10 border-rose-500/20 text-rose-300'} flex flex-col justify-between`}>
          <span className="text-slate-400 text-xs font-semibold uppercase tracking-wider mb-2">Saldo do Mês</span>
          <div>
            <span className="text-2xl font-bold">{fmoeda(totais.saldo)}</span>
            <p className={`text-xs mt-1 ${totais.saldo >= 0 ? 'text-emerald-400/80' : 'text-rose-400/80 font-semibold'}`}>
              {totais.saldo >= 0 ? 'Entradas cobrem os gastos do mês' : 'Gastos passaram as entradas'}
            </p>
          </div>
        </div>
      </section>
