// Testes das funções puras do roteador-central (updates simulados + CSVs sintéticos).
...
teste("comando com @bot e maiúsculas → reconhecido (/relatorio → rota relatorio)", () => {
  const r = classificarUpdate(msg({ text: "/Relatorio@SeuBot_bot" }), CTX);
...
// ─── detectarTipoCsv: discrimina pelo CONTEÚDO (cabeçalho), não pelo nome ──
teste("cabeçalho de fatura → cartao", () => {
  const t = "Data de Compra;Nome no Cartão;Final do Cartão;Categoria;Descrição;Parcela;Valor (em R$)\n05/05/2026;X;1;C;D;Única;10,00";
  assert.strictEqual(detectarTipoCsv(t), "cartao");
});

teste("cabeçalho de extrato (com BOM) → conta", () => {
  const t = "﻿EXTRATO DE CONTA CORRENTE C6 BANK\n\nData Lançamento,Data Contábil,Título,Descrição,Entrada(R$),Saída(R$),Saldo do Dia(R$)";
