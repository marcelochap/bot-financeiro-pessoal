...
## Limitações conhecidas (registradas no code review do build)
- **Corrida de duplo-clique (aceita, usuário único):** o roteador despacha
  `gerenciar-metas` com `waitForSubWorkflow:false` e a proteção contra duplo `gmok|`
  / `/novameta` homônimo é por leitura-antes-de-escrita do snapshot do `Ler Dados`.
  Dois eventos na janela de ~1–2 s (antes da 1ª escrita persistir) podem gerar 2
  linhas de Log ou 2 metas homônimas. Não serializado no n8n; risco prático mínimo
  com um só usuário. Endurecer (lock via `answerCallbackQuery` ou releitura) só se
  virar multi-usuário (fase 3+). O harness dry-run é sequencial e não exercita a
  corrida — o verde dos testes não cobre esse caminho.
