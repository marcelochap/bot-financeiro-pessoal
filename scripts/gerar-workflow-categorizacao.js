...
const codigoResolver = categorizadorSrc + [
  ...
  // Defesa (CLAUDE.md: 'parser nunca pode travar o workflow'): um nome de meta que
  // estoure os 64 bytes do callback_data (row com muitos dígitos + metaab|) não pode
  // abortar o lote inteiro do /categorizar — degrada para aviso sem teclado.
  "    let teclado, texto;",
  "    try {",
  "      teclado = montarTecladoMetas(l._row, metas);",
  "      texto = '🎯 Resgate de CDB — associar a qual meta?\\n' + textoPergunta;",
  "    } catch (e) {",
  "      teclado = { inline_keyboard: [] };",
  "      texto = '⚠️ Resgate de CDB (linha ' + l._row + '): não deu para montar o teclado de metas (' +",
  "        String((e && e.message) || e).slice(0, 140) + '). Categorize manualmente na planilha.\\n' + textoPergunta;",
  "    }",
  "    saida.push({ json: { fase: 'perguntar', row: l._row, texto, teclado,",
...
const codigoProcessar = categorizadorSrc + [
  ...
  "if (cb.tipo === 'meta' || cb.tipo === 'metaab') {",
  "  if (!metas.includes(cb.nome)) return [{ json: { acao: 'recusar', aviso: 'Meta não está mais ativa.' } }];",
  "  categoria = 'Meta: ' + cb.nome + (cb.tipo === 'metaab' ? ' (abatimento cdb)' : '');",
  "  idMeta = cb.nome;", // sem sufixo — preserva o match exato do /metas (gstack/specs/resgate-cdb-abatimento.md)
  "} else {",
